...
PKT_READ_FLASH      = [PROTOCOL, SERIAL_NBR, READ_DATA, 0x04, 0x04, 0x00]
PKT_WRITE_FLASH     = [PROTOCOL, SERIAL_NBR, WRITE_DATA, 0x08, 0x04, 0x00]

if __name__ == "__main__":

    baud = 0
    s = Serial(PORT_DEFAULT, baud, timeout=0.05)

    while s.baudrate < 1000000:
        s.baudrate += 1
        s.write(PKT_GET_VERSION)
        if len(s.read(5)) > 0:
            print('Found rate! %s' % s.baudrate)
            break
